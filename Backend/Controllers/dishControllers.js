...
import { getJSONFileData, getUniqueIngredients } from "./utils/utils.js";
import { read } from "fs";
...
export const getAllDishes = async (req, res) => {
  try {
    ...
    return res.status(200).json({ getDishes: dishesData });
  } catch (error) {
...
export const getDishById = async (req, res) => {
  try {
    ...
    return res.status(200).json({ dishDetails: dish });
  } catch (error) {
    console.error("Error reading file:", error);
    return res.status(500).json({ message: "Failed to load dish data" });
  }
...
export const getDishByIngredient = async (req, res) => {
  try {
    ...
    if (matchingDishes.length > 0) {
      ...
      res.status(200).json({
        message: "No dishes can be made with the provided ingredients.",
      });
    }
  } catch (error) {
...
export const getAllIngredients = async (req, res) => {
  try {
    const dishesData = await getJSONFileData();
    const ingredients = getUniqueIngredients(dishesData);
